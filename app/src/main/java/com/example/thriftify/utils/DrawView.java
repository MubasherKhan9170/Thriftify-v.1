...
import static com.example.thriftify.utils.DrawView.DrawViewAction.MANUAL_CLEAR;
import static com.example.thriftify.utils.DrawView.DrawViewAction.ZOOM;
...
public class DrawView extends View {
    ...
    private boolean bitmapStatus = false;
    ...
    public void setMenuButtons(MenuItem undoButton, MenuItem redoButton) {
    ...
    public void undo() {
        if (cuts.size() > 0) {
           ...
           if (cut.second != null) {
                ...
                Log.e(TAG, "Changed " + undoneCuts.size());
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (imageBitmap != null && currentAction != ZOOM) {
            if (!bitmapStatus) {
                ...
                Toast.makeText(context, "Can't perform any more operation due to low memory. Please undo to release the memory and try again.",Toast.LENGTH_SHORT).show();
            }
        }
    ...
    private void resizeBitmap(int width, int height) {
        if (width > 0 && height > 0 && imageBitmap != null) {
            imageBitmap = ImageUtils.getResizedBitmap(this.imageBitmap, width, height);
    ...
    public boolean getbitmapPoolingStatus(){
        return bitmapStatus;
    }
    ...
    /*This is an inner class for background processing using thread AsyncTask
    *
    *
    *
    *
    * */
    private static class AutomaticPixelClearingTask extends AsyncTask<Integer, Void, Bitmap> {
        private static final String TAG = "AutomaticPixelClearingT";
        ...
        protected Bitmap doInBackground(Integer... points) {
            ...
            try{
                ...
                drawViewWeakReference.get().bitmapStatus = true;
                bitmap = oldBitmap;
            }
